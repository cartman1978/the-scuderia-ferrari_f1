...
This website is for those who support the myth and legend of the Scuderia Ferrari. No other Formula 1 team carried the history, the passion for the romance that this great team does. It is a machine that consumes you. It is a family that embraces and protects you.

The Scuderia Ferrari F1 will make fans filter cars by year, find information related to drivers, engine size, races won, and images.
...
* The website has to work well on all kinds of devices like mobile phones, tablets, desktops.
* I want to have the possibility to filter f1 cars by year.
* I want to see cars specification and pictures of the model.
* The website has to be easy to use.
* I would like to read a brief story about each f1 car.
...
* As a user, I would like to be able to register for the website and have my personal environment.
* As a user, I would like to log in after I created my account and see previously inserted information about F1 car.
* As a user, I would like to have my profile with my favorite car.
* As a user, I want to be able to add car specs, race results, drivers, images for a specific car.
* As a user, I would like to have a dashboard where I can have a good overview.
* As a user, I want to be able to search Ferrari F1 car by year to get specific cars information.
* As a user, I want to be able to add many car information I want.
* As a user, I want to have the possibility to edit info added or want to add/delete some info.
* As a user, I want to have the possibility to delete a car as well when no longer relevant.
* As a user, I want the website to be easy to use.
* As a user, I want the process to add / edit/delete info to be easy.

### **Site owners Goals**

* To have an appealing website that Ferrari fans use to search for their best car model, fast lap, race won ecc.
* To have a great functionality and helps users to spend time reading and looking info about their favorite F1 car.
...
* Easy way to add car info to the dashboard.
* Easy way to search other users cars.
...
* To be able to filter cars by year.
...
To create the website structure I decide to use [Materialize](https://materializecss.com/).  Materialize provides various elements of CSS and Javascript which is very helpful to keep a good structure on your page and they offer various features like floating action button, Navbar, Tables etc.
...
* Possibility to add more cars for users
* CRUD Functions:
...
* Currently the user can only insert the image url. In the future, I would like that the user can upload an image from its computer and/or cloud accounts.
* Have a user profile with image/avatar and facts from the user.
* Have a 'forget password' functionality.
* Have a confirmed password to make sure the user has chosen the password he/she wanted.
* Add pagination so the list of the cars will be displayed with a max of 10 per page.
...
To create  a page where the user can register for its personal account to which only the user has access. initially my plan was to show the registration form when user open the website and to register before can visualize any car, but I changed my mind as I believe that it can appeal user to register if they like the content.
...
Sign-in form is working as planned and the input is being verified correctly with the stored information of the database. Redirection to the correct dashboard works as well as planned so the user can navigate to add a new car or view its profile and check what car was added.
...
I have created a form with the various input fields where the user can fill in the information. I made all the fields required because there are important car information that any other user need to see. For the image, I worked with an url that has to be filled in. For features to be implemented there is indeed the option to upload images from the user device.

- Test

I have tested the add_car form various times to make sure it works properly. The input is stored correctly in cars collection in the database. When testing this on mobile device I notice that the form was displayed badly as the gap between each field was to close, so I used a smaller column size in Materialize.
...
I would like to create a search form where user can simply enter the year who is looking to find a Ferrari F1, and all the results for the year entered will display to the user.

- Implementation

When the user visits the home page, a search button will appear on top where the user can enter the year which is looking for. If the date that was inserted is already created, the car will be displayed on the screen. If there was no car found with that date, the following message will appear: "No car found for this". After this, the user can either search on another date or return to the dashboard.

- Test

When I tested the first time I notice that if car was in the database it will be shown but if no car was found in the database, the above feedback message was not coming up, I had to change the expression if cars which I will explain on the Bugs section.

- Result

The search car page works as planned across various browsers and devices. The search function works as planned and returns the correct car by year or feedback message depending on the user input. 
...
The user has to be able to edit an existing car and profiles when a mistake was made or when the user wants to add certain information. So the information that the user inserted before remains intact and will be displayed on the page in order to have a good user experience. Every car profile will have an edit button that will take the user to the edit_car screen.
...
I have tested the edit functionality various times to make sure the data was being updated correctly. After the submission of the form, the user is being redirected to the home page and a message to confirm that the car was updated. The cancel button also takes the user back to the home page in case the user doesn't want to proceed with the changes.

- Result

The edit functionality works as planned across various browsers and devices. The correct document in the car database in being updated and the user is taken back to the main page. The cancel button redirects the user correctly to the relevant home page.
...
In case the car information is no longer relevant, the user should be able to delete it. There will be a delete button that the user can use. The relevant car will be removed from the database.
...
The delete functionality works as planned across various browsers and devices. The modal opens up when the button is clicked asking the user if they are sure they would like to delete the car. When no is selected, the user is taken back to the main page. When yes, the car is deleted from the database. The delete button for the car is correctly being displayed and works as planned.
...
- Bug
When the user searches for a car by year if the car is not in the database a message should appear to say that car was not found, but after the user hit the search button and no car results coming up I notice that no message was shown to the user.
...
- Verdict
After reloading the page and try again, messages are now showing correctly.
